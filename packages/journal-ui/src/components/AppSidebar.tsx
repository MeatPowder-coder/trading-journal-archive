...
import {
    JOURNAL_WEB_SIDEBAR_ROUTES,
    isWebSidebarRouteActive,
    type JournalIconKey,
...
const webIconMap: Record<JournalIconKey, ComponentType<{ className?: string }>> = {
    ...
    "user-round": Wallet,
    "wallet-cards": PieChart,
    "circle-dollar-sign": Receipt,
    "bell-ring": Bell,
    "message-square-more": MessageSquare,
};
...
const SidebarContent = ({ collapsed = false }: { collapsed?: boolean }) => {
    ...
    const routes = JOURNAL_WEB_SIDEBAR_ROUTES.map((route) => ({
        ...
        active: isWebSidebarRouteActive(route, pathname, currentTab),
    }));
                    ...
                    {routes.map((route) => (
                        ...
                        >
                            <div className="flex items-center">
                                <route.iconComponent className={cn("h-5 w-5 shrink-0", collapsed ? "" : "mr-3", route.active ? "text-blue-500" : "text-zinc-400 group-hover:text-blue-500")} />
